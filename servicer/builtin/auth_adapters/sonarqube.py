...
class AuthAdapter(BaseAuthAdapter):
    def __init__(self, config):
        ...
        self.scanner_os = self.config.get('scanner_os', 'linux')
        self.scanner_path = self.config.get('scanner_path', './')

        if self.scanner_path.endswith('/'):
            self.scanner_path = '%ssonar-scanner-cli-%s-%s' % (self.scanner_path, self.scanner_version, self.scanner_os)
    ...
    def ensure_install(self):
        if os.path.exists(self.scanner_path):
            ...
            download_file = 'sonar-scanner-cli-%s-%s.zip' % (self.scanner_version, self.scanner_os)
            ...
            #     self.run('rm -r %s' % download_path)
            self.run('unzip -d `dirname %s` %s' % (download_path, download_path))
            self.run('rm %s' % download_path)

            extracted_directory = 'sonar-scanner-%s-%s' % (self.scanner_version, self.scanner_os)
